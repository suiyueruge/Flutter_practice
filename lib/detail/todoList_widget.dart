...
class TodoListWidget extends StatelessWidget {
  final Widget body;
  final TodoDragContainer dragContainer;

  TodoListWidget({Key key, @required this.body, @required this.dragContainer})
      : assert(body != null),
...
typedef TOdoDragListener = void Function(
    double dragDistance, ScrollNotificationListener isDragEnd);

class TodoDragController {
  TOdoDragListener _dragListener;

  setDrag(TOdoDragListener l) {
    _dragListener = l;
  }
...
class TodoDragContainer extends StatefulWidget {
  final Widget drawer;
  ...
  TodoDragContainer(
      ...
      : assert(drawer != null),
        ...
        assert(height != null),
        assert(bottomHeight != null),
        super(key: key) {
    _controller = TodoDragController();
  }

  @override
  _TodoDragContainerState createState() => _TodoDragContainerState();
}

class _TodoDragContainerState extends State<TodoDragContainer>
...
TodoDragController _controller;
...
class TodoOverscrollNotificationWidget extends StatefulWidget {
  const TodoOverscrollNotificationWidget({
    Key key,
    @required this.child,
//    this.scrollListener,
  })  : assert(child != null),
...
//  final ScrollListener scrollListener;
  ...
  @override
  TodoOverscrollNotificationWidgetState createState() =>
      TodoOverscrollNotificationWidgetState();
}

/// Contains the state for a [TodoOverscrollNotificationWidget]. This class can be used to
/// programmatically show the refresh indicator, see the [show] method.
class TodoOverscrollNotificationWidgetState
    extends State<TodoOverscrollNotificationWidget>
    with TickerProviderStateMixin<TodoOverscrollNotificationWidget> {
